...
def main():
    ...
    parser.add_argument(
        '--with-parallels', '--parallels', action='store_true',
        help="Enable Parallels Guest Tools installation and overlays"
    )
    parser.add_argument(
    ...
    if args.with_parallels and profile == 'Vagrant':
    ...
    if args.with_parallels and args.parallels_dir:
        ...
        if os.path.isdir(abs_parallels_dir):
            ...
            if os.path.lexists(symlink_path):
                try:
                    ...
                    pass
            try:
                os.symlink(abs_parallels_dir, symlink_path)
                print(f"Linked parallels_iso directory into image description overlay at {symlink_path}")
            except Exception as e:
                print(f"Warning: Failed to create overlay symlink for parallels_iso: {e}")
    ...
    qemu_cmd += [
        ...
        '-device', f'virtio-9p-pci,id=fs0,fsdev=fsdev0,mount_tag=kiwidescription',
        
        # Share output bundle directory
        '-fsdev', f'local,security_model={out_security_model},id=fsdev1,path={abs_out_dir}',
        '-device', f'virtio-9p-pci,id=fs1,fsdev=fsdev1,mount_tag=kiwibundle'
    ]
    ...
    print("Launching native QEMU box VM in background...")
    
    # Hide QEMU boot noise unless verbose is requested
    qemu_stdout = None if args.verbose else subprocess.DEVNULL
    qemu_stderr = None if args.verbose else subprocess.DEVNULL
    ...
    # Wait for SSH port on 127.0.0.1:10022 to become available
    if not wait_for_ssh(10022, timeout=60):
        print("Error: Timeout waiting for VM SSH service to become available.")
        proc.kill()
        sys.exit(1)
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if connected:
            break
        print(f"VM SSH daemon is starting up (waiting for handshake, attempt {attempt}/{max_retries}). Retrying in 4 seconds...")
        time.sleep(4)
...
kiwi-ng --profile {profile} system build --description /description --target-dir /result --set-repo {args.repo_url} {extra_cmd}
